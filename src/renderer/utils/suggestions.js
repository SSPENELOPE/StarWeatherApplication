...
class SuggestionSettings {
  async getSuggestionSettings(profileId) {
    await fetch(
      `https://starweatherapi.azurewebsites.net/api/Edit/GetSuggestion/${profileId}`,
      {
        method: 'GET',
        headers: {
      ...
      .catch((error) => {
        toast.error(
          `${error}.. Sorry we messed up, if the problem persist please contact support`,
          {
            position: toast.POSITION.TOP_CENTER,
            draggable: false,
          }
  ...
  async setSuggestionSetting(profileId, suggestionType) {
    ...
    await fetch(
      'https://starweatherapi.azurewebsites.net/api/Edit/SetSuggestion',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
