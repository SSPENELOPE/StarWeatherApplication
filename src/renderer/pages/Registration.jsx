...
function Registration() {
  ...
  const checkIfExist = async (data) => {
    const response = await fetch(
      'https://starweatherapi.azurewebsites.net/auth/Authenticate',
      {
        method: 'GET',
        headers: {
  ...
  const handleRegistration = async (data) => {
    const response = await fetch(
      'https://starweatherapi.azurewebsites.net/auth/Registration',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      {data ? (
        ...
        <div className="regDiv mt-5">
          ...
          <form className="regForm" onSubmit={handleFormSubmit}>
            <label htmlFor="inputEmail4" className="align-self-start m-2">
              Email
            </label>
            <input
              type="email"
            ...
            <label htmlFor="inputName4" className="align-self-start m-2">
              Username
            </label>
            <input
              type="name"
            ...
            <label htmlFor="inputPassword4" className="align-self-start m-2">
              Password
            </label>
            <input
              type="password"
            ...
            <button type="submit" className="btn regBtn p-2 m-2">
              Register
            </button>
          </form>
        </div>
