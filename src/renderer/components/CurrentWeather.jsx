...
function CurrentWeather(props) {
  ...
  const { profileId, solidStar, regularStar, favorite, setFavorite } =
    props.favorite || [];
  ...
  const { current } = JSON.parse(localStorage.getItem('weatherData'));
  const { hourly } = JSON.parse(localStorage.getItem('weatherData'));
  ...
  if (profileId) {
    favoriteCities.forEach((item) => {
      // We added a protection measure for this in parent component
  ...
  const favoriteHandler = async () => {
    ...
    if (!newList.includes(city.toUpperCase().trim())) {
      ...
      const res = await fetch(
        'https://starweatherapi.azurewebsites.net/api/Favorites',
        {
          method: 'POST',
          headers: {
