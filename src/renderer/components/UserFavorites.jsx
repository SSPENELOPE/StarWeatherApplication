...
function UserFavorties(props) {
  ...
  const { favoriteCities, setFavoriteCities, handleFavoriteFetch } =
    props.cookieManager;
  ...
  const handleDelete = async (favId, favCity) => {
    ...
    await fetch(
      `https://starweatherapi.azurewebsites.net/api/Favorites/${favId}`,
      {
        method: 'DELETE',
        headers: {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          throw new Error(
            `Received ${response.status} ${response.statusText} from server`
          );
        }
      })
      .catch((error) => {
              ...
              <div>
                ...
                <ul className="d-flex flex-column">
                  {favoriteCities.map((item) => (
                    <li
                      key={item.FavId}
                      className="d-flex flex-row justify-content-between text-light"
                      id={`fav-item-${item.FavId}`}
                    >
                      <button
                        className="font btn text-light"
                        onClick={() => props.onClickButton(item.FavCity)}
                      >
                        {item.FavCity}
                      </button>
                      <button
                        className="font btn delete-btn text-light"
                        onClick={() => handleDelete(item.FavId, item.FavCity)}
                      >
                        <FontAwesomeIcon
              ...
              <div>
                ...
                <ul className="d-flex flex-column">
                  {favoriteCities.map((item) => (
                    <button
                      key={item.FavId}
                      className="font btn text-light cust-btn"
                      onClick={() => props.onClickButton(item.FavCity)}
                    >
                      {item.FavCity}
                    </button>
